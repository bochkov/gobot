...
func (r *repository) FindRegionByName(ctx context.Context, name string) ([]Region, error) {
	slog.Debug(fmt.Sprintf("search region by name = '%s'", name))
	...
	for rows.Next() {
		...
		if err = rows.Scan(&region.Id, &region.Name); err == nil {
			...
			if err != nil {
				slog.Warn(fmt.Sprintf("cannot get codes for region = %d", region.Id))
			} else {
				region.Codes = codes
			}
...
func (r *repository) FindRegionByCode(ctx context.Context, code string) (*Region, error) {
	slog.Debug(fmt.Sprintf("search region by code = '%s'", code))
